...
the available options and can be used to experiment along reading through the documentation.
...
The following table provides the schema of the configuration you need to know.
...
| `useIcon`
| boolean
| Use the SVG icon (from Simple Icons) instead of the text.
| Yes
|

| `links`
| array of objects
| The link of your social media.
| Technically yes but practically no :)
|

|===

With the `links` array, each object also follows a schema.
...
This theme doesn't have web feeds support but with the use of Hugo modules, it is possible to easily install and use one.
footnote:[It used to have until it was removed on v2.5.0 for centralizing the functionality on my link:https://github.com/foo-dogsquared/hugo-web-feeds/[custom web feed module instead].]

To start, you need to initialize your Hugo project as a Hugo module with `hugo mod init $HUGO_MOD_NAME` where `$HUGO_MOD_NAME` could be any name.

Then, edit your site configuration
